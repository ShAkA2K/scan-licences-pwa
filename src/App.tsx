...
import { CalendarPlus, FileDown, Users, Download, Calendar, ChevronRight } from 'lucide-react'
...
function seasonRange(label: string) {
  ...
  const start = new Date(Date.UTC(y1, 8, 1, 0, 0, 0))   // 1 Sept UTC
  const end   = new Date(Date.UTC(y2, 7, 31, 23, 59, 59)) // 31 Août UTC
...
function defaultSeasonLabel() {
  ...
  // Si avant septembre -> saison commence l'an dernier
...
async function exportSession(sessionId: string, fmt: ExportFormat) {
  // 1) entrées de la session
  const { data: entries, error: e1 } = await supabase
    .from('entries')
    .select('licence_no, timestamp, source_url')
    .eq('session_id', sessionId)
    .order('timestamp', { ascending: true })
  if (e1) throw e1

  // 2) résolve noms/prénoms
  ...
  if (licences.length) {
    ...
    if (e2) throw e2
    for (const m of M ?? []) members.set(m.licence_no, { last_name: m.last_name, first_name: m.first_name })
  }
...
export default function App() {
  ...
  const seasons = useMemo(() => {
    // On propose N-1, N, N+1 autour de la saison courante
    const now = new Date()
    const y = now.getUTCFullYear()
    const list: string[] = [`${y-1}-${y}`, `${y}-${y+1}`, `${y+1}-${y+2}`]
    // Ordre décroissant : saison courante en tête
    return list.reverse().sort().reverse()
  }, [])
  ...
  async function doExportSession(id: string) {
    try {
      await exportSession(id, selFmt)
    } catch (e: any) {
      alert('Export session: ' + (e?.message || String(e)))
    }
  }
  async function doExportSeason() {
    try {
      await exportSeason(selSeason, selFmt)
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-600 to-blue-700">
      ...
      <header className="bg-blue-700/60 backdrop-blur sticky top-0 z-30 border-b border-white/10">
        ...
        {/* Barre d’actions scrollable (mobile) */}
        <div className="mx-auto max-w-[720px] -mt-2 px-3 sm:px-6 pb-3">
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
            <button
              onClick={openSessionDuJour}
              disabled={busy}
              className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-3 py-2 text-white hover:bg-white/20 disabled:opacity-50"
            >
            ...
            <div className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-2 py-2 text-white">
              ...
              <select
                className="rounded-lg bg-transparent px-2 py-1 text-white outline-none"
                value={selSeason}
                onChange={e => setSelSeason(e.target.value)}
                title="Saison"
              >
                {/* saison courante + voisines */}
                {seasons.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
              <select
                className="rounded-lg bg-transparent px-2 py-1 text-white outline-none"
                value={selFmt}
                onChange={e => setSelFmt(e.target.value as ExportFormat)}
                title="Format"
              >
              ...
              <button
                onClick={doExportSeason}
                className="inline-flex items-center gap-1 rounded-lg bg-white/10 px-2 py-1 hover:bg-white/20"
                title="Exporter la saison"
              >
      ...
      <main className="mx-auto max-w-[720px] px-3 sm:px-6 py-6 space-y-8">
        ...
        <section className="rounded-2xl border border-white/20 bg-white/10 p-4 text-white">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              {sessionId
                ? <p className="text-sm text-white/80">Session: {sessionId}</p>
                : <p className="text-sm text-white/80">Aucune session en cours</p>}
            </div>
        ...
        <section className="space-y-3">
          ...
          <div className="grid sm:hidden grid-cols-1 gap-3">
            ...
            {!loadingSessions && !errSessions && sessions.length === 0 && (
              <div className="rounded-xl border bg-white px-3 py-4">Aucune session.</div>
            )}

            {sessions.map(s => (
              <div key={s.id} className="rounded-xl border bg-white p-3 flex items-center gap-3">
                <div className="min-w-0">
                  <div className="font-medium text-gray-900">Session du {formatParisDate(s.date)}</div>
                  <div className="text-xs text-gray-500 break-all">{s.id}</div>
                </div>
                <div className="ml-auto flex items-center gap-2">
                  <button
                    onClick={() => doExportSession(s.id)}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {!loadingSessions && !errSessions && sessions.length === 0 && (
                    <tr><td className="px-3 py-4" colSpan={3}>Aucune session.</td></tr>
                  )}
