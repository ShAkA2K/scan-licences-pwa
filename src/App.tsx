...
function Avatar({ member }: { member: Member }) {
  ...
  const candidates = React.useMemo(() => buildPhotoCandidates(member), [member.photo_url, member.licence_no])
  const [idx, setIdx] = React.useState(0)

  React.useEffect(() => { setIdx(0) }, [member.photo_url, member.licence_no])
...
// ------- Export helpers -------
type ExportRow = { last_name: string; first_name: string; licence_no: string; created_at: string }
async function exportRows(format: ExportFormat, filename: string, rows: ExportRow[]) {
  try {
